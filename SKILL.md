...
### 1. 主 agent 提问（Intent Preflight）

拿到用户问题，先做一次**预检**：判断它**连同附件能否自洽说明要做什么**——核心永远是问题本身（含附带的图片/文件），补背景只是为了让 web AI 看懂，不要不分场景套模板。预检后按**三态路由**处置，绝不在"缺的部分只有用户知道"时静默脑补。

**自检标准**：一个完全没参与过这次任务、但能看到这条消息全部内容（含附件）的人，能否明白要做什么、为什么问、期待什么答案？能 → 直接发；不能 → 判断"缺的部分能否由主 agent 合理补上"，再走下方路由。

**🟢 直接派发** —— 问题能自洽，或缺口存在明确行业默认（如"2026 Java 后端技术趋势"缺水平/范围但有稳定默认），或用户要求快速探索。无需确认，直接派发。

**🟡 先回述再派发（默认兜底）** —— 问题依赖项目/任务背景，或缺的部分可合理推断但补全会修正/具体化原意时：补齐缺口后，把**规范化后的问题**写回用户确认：
> 我将按以下理解执行：〔规范化问题，含默认假设与不含项〕。若无异议 N 秒后自动继续；如需修改请直接指出。

非阻塞：用户不回复自动继续，保持一键感。**禁止把脑补包装成默认假设**——高影响假设必须可见、可改、可否决。

**🔴 主动反问（阻断派发）** —— 命中任一即停止派发，向用户提问（最多一轮、≤3 个问题、优先选择题并给推荐默认项）：
- 指代悬空：含代词（它/这个/上述/该项目）且当前对话无法唯一消解（尤其全新对话）
- 显性逻辑矛盾：条件互斥且无优先级（"要最便宜又要全功能顶配"）
- 关键实体缺失：连研究对象都无法可靠推断
- 歧义导致方向分叉：不同解释指向相反研究结论（"苹果的最新策略"——公司/水果/电影）
- 高风险不可逆决策缺关键约束（法律/合规/医疗/重大架构选型）

**派发契约**：派发给 web AI 的是**规范化后的问题（统一 Brief）**，不是用户原话——经回述确认的用确认后版本。若回述/反问后仍存在默认假设，假设必须随问题一并列出，最终文档在"背景"或"AI Research Notes"中披露。
...
原因：Claude Code 拥有项目上下文，但网页端 AI 是白纸——用户的实际提问默认带了上下文，这个上下文 web AI 看不到，所以主 agent 要自行判断补什么；若缺的只有用户知道，必须反问而不是脑补。
